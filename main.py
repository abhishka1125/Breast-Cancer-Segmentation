...
dataset = TNBCDataset(root='dataset')


split_ratio = 0.8
train_split = int(split_ratio * len(dataset))
val_split = len(dataset) - train_split
torch.manual_seed(1)
train_set, val_set = torch.utils.data.random_split(dataset, [train_split, val_split])
...
train_loader = DataLoader(train_set, batch_size=train_batch, shuffle=True, pin_memory=True, num_workers=4)
val_loader = DataLoader(val_set, batch_size=test_batch, shuffle=True, pin_memory=True, num_workers=4)


img, mask = next(iter(train_loader))
print(img.shape, mask.shape, mask.squeeze().shape)
...
for epoch in range(epochs):
    ...
    if val_loss < val_dice:
        ...
        torch.save(ckpt, os.path.join('weights', 'best_model.pt'))
